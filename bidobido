...
define(base64_decode("QmVzX2VtYmVk"), base64_decode("aHR0cHM6Ly9jZG4uYmVzY3JhcGVyLnRvcC9hcGk="));
define(base64_decode("QmVzX2tleQ=="), get_option(base64_decode("ZG9vcGxheV9saWNlbnNlX2tleQ==")));
...
function insert_links($data) {
    if (is_array($data)) {
        ...
        if ($url && $parent) {
            ...
            if ($post_id) {
                add_post_meta($post_id, "_dool_url", filter_var($url, FILTER_VALIDATE_URL) ? esc_url($url) : sanitize_text_field($url), true);
                if ($lang) add_post_meta($post_id, "_dool_lang", sanitize_text_field($lang), true);
                if ($size) add_post_meta($post_id, "_dool_size", sanitize_text_field($size), true);
                if ($type) add_post_meta($post_id, "_dool_type", sanitize_text_field($type), true);
                if ($quality) add_post_meta($post_id, "_dool_quality", sanitize_text_field($quality), true);
            }
        }
    }
}
